...
class StartScreen:
    ...
    def __init__(self,m,d):
        ...
        m.log.write('[DEBUG] Инициализация класса Display.StartScreen')
        
        self.colors = d.colors['startscreen']
        self.colors['grad-bg'] = d.gradient(m,self.colors['grad-bg'][0],self.colors['grad-bg'][1],120)
        self.colors['grad-button-text'] = d.gradient(m,self.colors['grad-button-text'][0],self.colors['grad-button-text'][1],30)
        ...
        if m.Disp.anim == 'startscreen_start':
            ...
            if m.Disp.fps != 0:
                ...
                if m.Disp.anim == 'startscreen_start2':
                    self.offset = [i / (1 + 0.06*(60/sm)) for i in self.offset]
